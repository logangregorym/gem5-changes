...
void TraceBasedGraph::predictValue(Addr addr, uint16_t uopAddr, uint64_t value, uint64_t confidence, unsigned latency)
{
    ...
    // Check to see if we have enough space in trace queue
    if (candidateTraceQueue.size() > 64)
    {
        DPRINTF(SuperOp, "Not enough space in trace queue! Trace Queue Size: %d\n", candidateTraceQueue.size());
        return;
    }
    ...
    for (uint64_t way=0; way < 8; way++) {
        ...
        if (decoder->uopValidArray[idx][way] && decoder->uopTagArray[idx][way] == tag)
        {
            // get the maximum hotness. Later will be used
    ...
    if (newTrace.hotness < 7 || newTrace.length < 4) { // TODO: revisit: pretty low bar
        ...
        for (auto &lv1: newTrace.superOptimizedTrace)
        {
            if (lv1.second[0].macroop->isMacroop())
            {
                lv1.second[0].macroop->deleteMicroOps(); 
    ...
    newTrace.setOrigHeadAndEndAddr(FullUopAddr(newTrace.originalTrace.begin()->first, 0), 
                                  FullUopAddr(newTrace.originalTrace.rbegin()->first, newTrace.originalTrace.rbegin()->second.size()-1));
    ...
    for (auto it = traceMap.begin(); it != traceMap.end(); it++) {
        ...
        if ((newTrace.getOrigHeadAddr().pcAddr != it->second.getOrigHeadAddr().pcAddr) || (newTrace.getOrigEndAddr().pcAddr != it->second.getOrigEndAddr().pcAddr)) {
            continue;
        }
        ...
        for (int i=0; i < SpecTrace::NUM_PREDICTION_SOURCES; i++) {
                ...
                it->second.source[i].value != value) 
            {
                ...
                DPRINTF(SuperOp, "Trace Map already holds a trace for this head and end addresses but the predicted value is different! HeadAddr = %#x EndAddr = %#x Trace Predicted Value: %#x New Predicted Value: %#x\n", 
                                  it->second.getOrigHeadAddr().pcAddr, it->second.getOrigEndAddr().pcAddr, it->second.source[i].value, value);
        ...
        for (auto const & lv2: lv1.second)
        {
                ...
                {   
                    if (ret.confidence >= 5){
                        ...
                        pred_source_idx++;
                        if (pred_source_idx == SpecTrace::NUM_PREDICTION_SOURCES) break;
                    }
                }      
    ...
    newTrace.setTraceID(SpecTrace::traceIDCounter++);
    newTrace.state = SpecTrace::QueuedForFirstTimeOptimization;
    traceMap[newTrace.getTraceID()] = newTrace;
    candidateTraceQueue.push(newTrace.getTraceID());
...
bool TraceBasedGraph::selectNextTraceForsuperOptimization()
{
    ...
    while(!candidateTraceQueue.empty()) {
        ...
        else 
        {
            ...
            assert(currentTraceIDGettingSuperOptimized);
            assert(currentTraceIDGettingSuperOptimized == trace_it->second.getTraceID());
    ...
    // in case we can't find a valid trace to super optimize (all the traces where evicted or the candidate trace queue is empty)
...
void TraceBasedGraph::finalizeSuperOptimizedTrace()
{
    ...
    [&superHeadAddr, trace_it] {
        for (auto addr_it = trace_it->second.superOptimizedTrace.begin(); addr_it != trace_it->second.superOptimizedTrace.end(); addr_it++)
        {
            for (auto micro_it = addr_it->second.begin(); micro_it != addr_it->second.end(); micro_it++)
            {
                if (!micro_it->second.compacted)
                {
                    superHeadAddr = FullUopAddr(addr_it->first,micro_it->first);
                    return;
                }
            }
        ...
        for (auto addr_it = trace_it->second.superOptimizedTrace.rbegin(); addr_it != trace_it->second.superOptimizedTrace.rend(); addr_it++)
        {
            for (auto micro_it = addr_it->second.rbegin(); micro_it != addr_it->second.rend(); micro_it++)
            {
                if (!micro_it->second.compacted)
                {
                    ...
                    superEndAddr = FullUopAddr(addr_it->first,micro_it->first);
                    return;
                }
            }
    ...
    bool updateSuccesfull = decoder->specCache->AddToSpeculativeCache(trace_it->second.getOrigHeadAddr().pcAddr, trace_it->second.getTraceID(), trace_it->second.shrunkLength, evictedTraces);
    assert(updateSuccesfull);
    decoder->specCache->DumpSpecCache();
    ...
    // Just to make sure that all the completed traces in traceMap are equal to spec cache traces
    // This is just a sanity check
    if (traceMap.size() != (candidateTraceQueue.size() + decoder->specCache->GetNumOfValidTracesInSpecCache()))
    {
        std::cout << traceMap.size() << "::::"  << candidateTraceQueue.size() << "::::" << decoder->specCache->GetNumOfValidTracesInSpecCache() << std::endl;

        std::cout << "Traces in traceMap: " ;
        for (auto const& tr : traceMap)
        {
            std::cout << tr.first << " ";
        }
        std::cout << "\n";

        std::cout << "Traces in Candidate Queue: " ;
        while (!candidateTraceQueue.empty())
        {
            std::cout << candidateTraceQueue.front() << " ";
            candidateTraceQueue.pop();
        }
        std::cout << "\n";

        assert((traceMap.size() == (candidateTraceQueue.size() + decoder->specCache->GetNumOfValidTracesInSpecCache())));
    }
    ...
    DPRINTF(TraceGen,"#########################################################################################################################################\n");
    DPRINTF(TraceGen,"******************************* ORIGINAL TRACE START [%#x][%d] ***************************************\n",  trace_it->second.getOrigHeadAddr().pcAddr, trace_it->second.getOrigHeadAddr().uopAddr) ;
    for (auto const &lv1: trace_it->second.originalTrace)
    {
            for (auto const &lv2: lv1.second)
            {
    ...
    DPRINTF(TraceGen, "-------------------------------- ORIGINAL TRACE END [%#x][%d] -----------------------------------------\n",  trace_it->second.getOrigEndAddr().pcAddr, trace_it->second.getOrigEndAddr().uopAddr) ;
    // assert(trace_it->second.traceHeadAddr == trace_it->second.endAddr.pcAddr);
    DPRINTF(TraceGen,"-------------------------------- SUPEROPTIMIZED TRACE START [%#x][%d] -----------------------------------------\n",  trace_it->second.getSuperHeadAddr().pcAddr,  trace_it->second.getSuperHeadAddr().uopAddr) ;
    for (auto const &lv1: trace_it->second.superOptimizedTrace)
    {
        for (auto const &lv2: lv1.second)
        {
    ...
    DPRINTF(TraceGen, "******************************* SUPEROPTIMIZED TRACE END [%#x][%d] ***************************************\n",  trace_it->second.getSuperEndAddr().pcAddr, trace_it->second.getSuperEndAddr().uopAddr) ;
    DPRINTF(TraceGen,"#########################################################################################################################################\n");
...
bool TraceBasedGraph::updateSpecTrace(TraceMap::iterator& _trace_it, StaticInstPtr _decodedMicroOp , bool propagated, bool isPredSource) {
    ...
    if (isDeadCode) 
    {
        DPRINTF(ConstProp, "Dead code at %#x:%d\n", macro_addr, micro_addr);
        return true;
    }
    ...
    {
        DPRINTF(ConstProp, "Instruction at %#x:%d is not a dead code as it's not propagated!\n", macro_addr, micro_addr);
    }
    else if (_decodedMicroOp->isReturn())
    {
...
bool TraceBasedGraph::generateNextSuperOptimizedTraceInst() {
    ...
    assert(trace_it != traceMap.end());
    assert(trace_it->second.state == SpecTrace::OptimizationInProcess);
    assert(trace_it->second.getTraceID() == currentTraceIDGettingSuperOptimized);
    ...
    if (isPredictionSource(trace_it, value, confidence, latency)) {
        assert(type != "rdip" && type != "limm" && type != "movi");
        ...
        for (int i = 0; i < decodedSuperMicroop->numDestRegs(); i++) {
            ...
            if (destReg.classValue() == IntRegClass) {
                ...
                decodedSuperMicroop->predictedValue = value;
                // set this as a prediction source, therefore we need to check for missprediction
                decodedSuperMicroop->setTracePredictionSource(true);
                // only predictions coming from uop cache should have this set to diffrentiate between spec and uop predictions
                decodedSuperMicroop->predictedLoad = false; 
            }
        }
    ...
    if (decodedSuperMicroop->isControl()) {
        for (int i=0; i<16; i++) { // 16 int registers
            if (regCtx[i].valid && !regCtx[i].source) {
    ...
    DPRINTF(ConstProp, "Live Outs:\n");
    for (int i=0; i<16; i++) {
        if (regCtx[i].valid && !regCtx[i].source)
            DPRINTF(ConstProp, "reg[%i]=%#x\n", i, regCtx[i].value);
    }
    if (ccValid) {
        DPRINTF(ConstProp, "PredccFlagBits: %#x\n", PredccFlagBits);
        DPRINTF(ConstProp, "PredcfofBits: %#x\n", PredcfofBits);
        DPRINTF(ConstProp, "PreddfBit: %#x\n", PreddfBit);
        DPRINTF(ConstProp, "PredecfBit: %#x\n", PredecfBit);
        DPRINTF(ConstProp, "PredezfBit: %#x\n", PredezfBit);
    } else {
        DPRINTF(ConstProp, "No live out CC\n");
    }
