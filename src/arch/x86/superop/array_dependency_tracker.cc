...
void ArrayDependencyTracker::addToGraph(StaticInstPtr uop, Addr addr, unsigned uopAddr)
{
    ...
    for (int i = 0; i < uop->numSrcRegs(); i++) {
        ...
        if (srcReg.isIntReg() || srcReg.isFloatReg() || srcReg.isCCReg() || srcReg.isVecElem()) {
            if (registerValidMap[srcReg.flatIndex()]) {
		// TODO: else, dummy source
		...
		// Add the path to this inst's producers; first uop...
                unsigned prodIdx = 0;
                bool found = false; // reusing
                while (prodIdx < 256 && !found) {
                    if (microopDependencyGraph[idx][uopway][uopAddr].producers[prodIdx] == 0) {
                        found = true;
                        microopDependencyGraph[idx][uopway][uopAddr].producers[prodIdx] = -1; // Change
                    } else {
                        prodIdx++;
                    }
                }
                ...
                while (prodIdx < 256 && !found) {
                    if (speculativeDependencyGraph[idx][specway][uopAddr].producers[prodIdx] == 0) {
                        found = true;
                        speculativeDependencyGraph[idx][specway][uopAddr].producers[prodIdx] = -1; // Change
                    } else {
                        prodIdx++;
                    }
                }
    ...
    for (int i = 0; i < uop->numDestRegs(); i++) {
        ...
        if (destReg.isIntReg() || destReg.isFloatReg() || destReg.isCCReg() || destReg.isVecElem()) {
        ...
        for (int i2=0; i2<8; i2++) {
            for (int i3=0; i3<6; i3++) {
                if (decoder->speculativeValidArray[i1][i2] && decoder->speculativeCache[i1][i2][i3]) {
					StaticInstPtr inst = decoder->speculativeCache[i1][i2][i3];
					// X86ISA::ExtMachInst emi = inst->machInst;
					// cout << "opcode: " << emi.opcode.op << endl;
					//DependGraphEntry entry = speculativeDependencyGraph[i1][i2][i3];
                    string type = inst->getName();
					if (type == "mov") {
						// printf("%s\n", inst->instMnem);
						// cout << emi.opcode.op << endl;
						uint8_t srcs = inst->numSrcRegs();
						if (srcs == 3) {
							// Can forward!
							for (int i=0; i < srcs; i++) {
								// printf("source %i is reg %u of type %s\n", i, inst->srcRegIdx(i).index(), inst->srcRegIdx(i).className());
							}
						} else if (srcs == 8) {
							// cmov, skipping for now
						} else {
							printf("Unrecognized source count %i\n", srcs);
						}
					} else if (type == "movi") {
					...
					// if (type == "mov") {
						// Check if all ready
						// if so, set dest path based on source path
						// uint8_t srcs = inst->numSrcRegs();
						// cout << "Number of Source Registers: " << srcs << endl;
						// for (int i=0; i < srcs; i++) {
							//printf("source %i is reg %u of type %s\n", i, inst->srcRegIdx(i).index(), inst->srcRegIdx(i).className());
						// }
					// }
...
void ArrayDependencyTracker::measureChain(FullUopAddr addr, unsigned recursionLevel, vector<FullUopAddr>& checked)
{
    ...
    if (decoder->speculativeCache[idx][way][uopAddr]) {
	if ((decoder->speculativeCache[idx][way][uopAddr]->isControl() || decoder->speculativeCache[idx][way][uopAddr]->isCall() || decoder->speculativeCache[idx][way][uopAddr]->isReturn() || decoder->speculativeCache[idx][way][uopAddr]->isDirectCtrl() || decoder->speculativeCache[idx][way][uopAddr]->isIndirectCtrl() || decoder->speculativeCache[idx][way][uopAddr]->isCondCtrl() || decoder->speculativeCache[idx][way][uopAddr]->isUncondCtrl() || decoder->speculativeCache[idx][way][uopAddr]->isCondDelaySlot())) {
        branchesOnChains++;
    ...
    for (int i=0; i<256; i++) {
        if (speculativeDependencyGraph[idx][way][uopAddr].producers[i] != 0) {
            if (speculativeDependencyGraph[idx][way][uopAddr].producers[i] != -1 && connections[speculativeDependencyGraph[idx][way][uopAddr].producers[i]].valid) {
                allReady++;
            }
        }
    }
    if (decoder->speculativeCache[idx][way][uopAddr] && allReady == decoder->speculativeCache[idx][way][uopAddr]->numSrcRegs() && allReady != 0) {
        reducableInstCount++;
        if (!speculativeDependencyGraph[idx][way][uopAddr].seen) {
            totalReducable++;
            speculativeDependencyGraph[idx][way][uopAddr].seen = true;
